...
<head>
  ...
  <style>
    ...
    .header p { margin: 4px 0 0; opacity: 0.85; font-size: 13px; text-transform: capitalize; }
    .body { padding: 24px 28px; }
    .section { margin-bottom: 24px; }
    .section-title { font-size: 13px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px; color: #6B7280; margin-bottom: 10px; }
    ...
    .stat { flex: 1; background: #F8FAFC; border-radius: 8px; padding: 14px; text-align: center; }
...
<body>
  <div class="container">
    ...
    <div class="body">
      ...
      <div class="stats">
        <div class="stat">
          <div class="stat-num" style="color:#EF4444">${urgentes.length}</div>
          <div class="stat-label">Urgentes</div>
        </div>
        <div class="stat">
          <div class="stat-num" style="color:#F97316">${enRetard.length}</div>
          <div class="stat-label">En retard</div>
        </div>
        <div class="stat">
          <div class="stat-num" style="color:#3B82F6">${taches.length}</div>
          <div class="stat-label">À faire</div>
        </div>
        <div class="stat">
          <div class="stat-num" style="color:#F59E0B">${charges.length}</div>
          <div class="stat-label">Charges dues</div>
        </div>
      </div>
      ...
      <div class="section">
        ...
        ${urgentes.length === 0
          ...
          <div class="item item-urgent">
            <div class="item-title">${t.titre}<span class="badge badge-urgent">${t.priorite}</span></div>
            ${t.deadline ? `<div class="item-meta">Deadline : ${format(parseISO(t.deadline), "HH'h'mm, d MMM", { locale: fr })}</div>` : ''}
          </div>`).join('')}
      </div>
      ...
      ${enRetard.length > 0 ? `
      <div class="section">
        <div class="section-title">⚠️ En retard</div>
        ${enRetard.map(t => `
        <div class="item item-retard">
          <div class="item-title">${t.titre}<span class="badge badge-retard">Retard</span></div>
          ${t.deadline ? `<div class="item-meta">Était prévu le ${format(parseISO(t.deadline), "d MMM yyyy", { locale: fr })}</div>` : ''}
        </div>`).join('')}
      </div>` : ''}
      ...
      <div class="section">
        ...
        <div class="item item-normal">
          <div class="item-title">${t.titre}</div>
        </div>`).join('')}
      </div>` : ''}
      ...
      <div class="section">
        ...
        ${aFaire.length === 0
          ...
          <div class="item item-normal">
            <div class="item-title">${t.titre}</div>
            ${t.deadline ? `<div class="item-meta">Deadline : ${format(parseISO(t.deadline), "d MMM yyyy", { locale: fr })}</div>` : ''}
          </div>`).join('')}
      </div>
      ...
      <div class="section">
        ...
        <div class="item item-charge">
          ...
          <div class="item-meta">${c.montant.toLocaleString('fr-TN', { minimumFractionDigits: 3 })} DT${c.date_echeance ? ` — Échéance : ${format(parseISO(c.date_echeance), 'd MMM yyyy', { locale: fr })}` : ''}</div>
        </div>`).join('')}
      </div>` : ''}
    </div>

    <div class="footer">
      FlowTask — ${format(maintenant, "d MMMM yyyy 'à' HH'h'mm", { locale: fr })}
    </div>
  </div>
</body>
</html>`
